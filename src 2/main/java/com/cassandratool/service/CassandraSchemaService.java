...
public final class CassandraSchemaService {
    ...
    /** Текстовое описание keyspace (CQL + основные опции). */
    public static Optional<String> describeKeyspaceForUi(CqlSession session, String keyspace) {
        if (keyspace == null || keyspace.isBlank()) {
            return Optional.empty();
        }
        return session.getMetadata()
                ...
                .map(k -> {
                    ...
                    sb.append(k.describe(true)).append("\n\n");
                    sb.append("durable_writes: ").append(k.isDurableWrites()).append("\n");
                    sb.append("virtual: ").append(k.isVirtual()).append("\n");
                    sb.append("replication: ").append(k.getReplication()).append("\n");
                    sb.append("user_types: ").append(k.getUserDefinedTypes().size()).append("\n");
                    sb.append("functions: ").append(k.getFunctions().size()).append("\n");
                    sb.append("aggregates: ").append(k.getAggregates().size()).append("\n");
                    return sb.toString();
                });
    }

    /** Полное описание таблицы (DDL и дочерние объекты, если есть). */
    public static Optional<String> describeTableForUi(CqlSession session, String keyspace, String table) {
        return findTable(session, keyspace, table).map(t -> t.describeWithChildren(true));
    }
}
